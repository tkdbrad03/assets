...
firebase.auth().onAuthStateChanged(user => {
  ...
  if (user) { currentUser = user; showApp(user); subscribeToData(); }
  else { currentUser = null; showLogin(); [unsubLoads,unsubReceipts,unsubMaints,unsubInvoices].forEach(u=>u&&u()); }
});
...
window.signInWithGoogle = function() {
  ...
  firebase.auth().signInWithPopup(provider).catch(e => {
    ...
    } else showToast('Sign in failed: '+e.code,'#D62828');
  });
};

window.signOut = function() { firebase.auth().signOut(); document.getElementById('signout-menu').classList.remove('open'); };

function showLogin() {
  document.getElementById('login-screen').style.display='flex';
  ['app-header','scroll-area','fab','bottom-nav'].forEach(id=>document.getElementById(id).style.display='none');
}
...
function showApp(user) {
  ...
  if (user.photoURL) av.outerHTML=`<img src="${user.photoURL}" class="user-avatar" id="user-avatar" onclick="toggleSignoutMenu()" alt="avatar">`;
  else av.textContent=(user.displayName||user.email||'C')[0].toUpperCase();
...
function subscribeToData() {
  ...
  unsubReceipts = userColl('receipts').orderBy('createdAt','desc').onSnapshot(snap => {
    receipts = snap.docs.map(d=>({id:d.id,...d.data()})); renderReceipts();
  });
  unsubMaints = userColl('maints').orderBy('createdAt','desc').onSnapshot(snap => {
    maints = snap.docs.map(d=>({id:d.id,...d.data()})); renderMaintenance();
  });
...
window.setFilter=function(val,el){
  ...
  el.classList.add('active'); renderLoads();
};
...
window.saveInvoiceToHistory=function(){
  const p=window._pendingInvoice; if(!p){showToast('Generate an invoice first','#D62828');return;}
...
window.closeInvoiceHistory=function(){document.getElementById('invoice-history-backdrop').classList.remove('open');};
window.closeInvoiceHistoryIfBackdrop=function(e){if(e.target===document.getElementById('invoice-history-backdrop'))window.closeInvoiceHistory();};
...
window.printInvoice=function(){
  const inv=document.getElementById('invoice-preview-content'); if(!inv)return;
...
window.handleReceiptUpload=function(e){
  ...
  reader.onload=ev=>{
    pendingReceiptImage=ev.target.result; editingReceiptId=null;
...
function closeReceiptSheet(){document.getElementById('receipt-sheet-backdrop').classList.remove('open');editingReceiptId=null;pendingReceiptImage=null;}
